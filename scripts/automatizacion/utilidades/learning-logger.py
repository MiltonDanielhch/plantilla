...
class LearningLogger:
        ...
        for ext in ['*.py', '*.js', '*.ts']:
            for filepath in Path('.').rglob(ext):
                ...
                try:
                    ...
                    imp_matches = re.findall(r'from\s+(\w+)\s+import|import\s+(\w+)', content)
    ...
    def generate_insights(self):
        ...
        if top_imports:
            insights.append(f"Imports más usados: {', '.join(list(top_imports.keys())[:5])}")
        
        top_funcs = dict(self.data['patterns'].get('top_functions', []))
        common_funcs = [f for f, c in sorted(top_funcs.items(), key=lambda x: x[1], reverse=True)[:5]]
        if common_funcs:
            insights.append(fFUNCIONES más comunes: {', '.join(common_funcs)}")
    ...
    def learn_from_user(self):
        ...
        for line in git_log.split('\n'):
            if line:
                words = line.split()
                if len(words) > 1:
                    commit_types[words[1] if words[0].startswith('[') else 'update'] += 1
