...
def generate_prometheus_format(metrics):
    ...
    lines.append(f"sintonia_memory_percent {metrics['memory']['memory_percent']} {timestamp}")
    
    lines.append(f"# HELP sintonia_disk_percent Disk usage percentage(f"# TYPE sint")
    lines.appendonia_disk_percent gauge")
    
    for disk in metrics['disk']:
        mountpoint = disk['mountpoint'].replace('/', '_').replace('.', '_')
        lines.append(f"sintonia_disk_percent{{mountpoint=\"{disk['mountpoint']}\"}} {disk['percent']} {timestamp}")
    
    lines.append(f"# HELP sintonia_network_bytes Network I/O bytes")
    lines.append(f"# TYPE sintonia_network_bytes counter")
    lines.append(f"sintonia_network_bytes_sent {metrics['network']['bytes_sent']} {timestamp}")
    lines.append(f"sintonia_network_bytes_recv {metrics['network']['bytes_recv']} {timestamp}")
